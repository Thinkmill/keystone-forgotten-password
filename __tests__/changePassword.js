...
test('should handle expired change-password request', () => {
	...
	.expect(400)
	.then((response) => {
		expect(response.body).toEqual({ success: false, errors: { general: 'Request to reset password has expired' } });
	});
...
test('should handle invalid password', () => {
	...
	.expect(400)
	.then((response) => {
		expect(response.body).toEqual({ success: false, errors: { password: 'Password is required' } });
	});
...
test('should call onChangePasswordEmail handler', () => {
	...
	.then((response) => {
		...
		expect(onChangePasswordEmail).toBeCalledWith({ email: 'test@test.com', id: '1234', passwordLastUpdated: TIME_STAMP, recipientEmail: 'test@test.com' });
	});
});
