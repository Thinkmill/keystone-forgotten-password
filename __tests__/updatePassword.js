...
test('change password password 400 on no existing password', () => {
	...
	.expect(400)
	.then((response) => {
		expect(response.body).toEqual({ success: false, errors: { existingPassword: 'Please enter an existing password' } });
	});
});
...
test('Password required', () => {
	...
	.expect(400)
	.then((response) => {
		expect(response.body).toEqual({ success: false, errors: { password: 'Please enter a new password' } });
	});
});
...
test('New password validation error ', () => {
	...
	.expect(400)
	.then((response) => {
		expect(response.body).toEqual({ success: false, errors: { password: 'use at least one upper case character' } });
	});
});
